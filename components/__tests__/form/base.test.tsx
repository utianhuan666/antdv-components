import { FontSizeOutlined } from '@antdv-next/icons'
import { Button, ConfigProvider, Input } from 'antdv-next'
...
import { act, cleanup, fireEvent, render, userEvent, waitFor, waitForWaitTime } from '../testUtils'
...
describe('ProForm', () => {
  ...
  it('📦 support formRef', async () => {
    ...
    expect(formRef.value?.getFieldFormatValue?.('test')?.join('-')).toBe(
      '12-34',
    )
    expect(
      formRef.value?.getFieldFormatValueObject?.('test')?.test.join('-'),
    ).toBe('12-34')
    expect(formRef.value?.getFieldFormatValueObject?.()?.test.join('-')).toBe(
      '12-34',
    )
    expect(formRef.value?.getFieldsFormatValue?.()?.test.join('-')).toBe(
      '12-34',
    )
    expect(formRef.value?.getFieldFormatValue?.(['test'])?.join('-')).toBe(
      '12-34',
  ...
  it('📦 ProFormCaptcha support value and onchange', async () => {
    const onFinish = vi.fn()
    const wrapper = render(
      <ProForm onFinish={(values: any) => onFinish(values.name)}>
        <ProFormCaptcha
          onGetCaptcha={async () => {
    ...
    const wrapper = render(
      <ProForm>
        <ProFormCaptcha
          ...
          captchaTextRender={(timing: boolean) => (timing ? '重新获取' : '获取')}
          captchaProps={{
            id: 'test',
          }}
          label="name"
  ...
  it('📦 ProForm.Group support collapsible', async () => {
    ...
    const wrapper = render(
      <ProForm>
        <ProForm.Group title="qixian" collapsible onCollapse={(c: boolean) => fn(c)}>
  ...
  it('📦 validateFieldsReturnFormatValue', async () => {
    ...
    const App = {
      setup() {
        ...
        onMounted(() => {
          formRef.value?.validateFieldsReturnFormatValue?.().then((val: any) => {
            fn1(val.date)
          })
        })
  ...
  it('📦 when dateFormatter is a Function', async () => {
    ...
    const App = {
      setup() {
        return () => (
          <ProForm
            dateFormatter={(value: any, valueType: any) => {
  ...
  it('📦 getFieldsFormatValue should work correctly', async () => {
    ...
    const allValues = formRef.value?.getFieldsFormatValue?.()
    ...
    const allDataValues = formRef.value?.getFieldsFormatValue?.(true)
    ...
    const nameValue = formRef.value?.getFieldFormatValue?.('name')
    expect(nameValue).toBe('test')

    const ageValue = formRef.value?.getFieldFormatValue?.('age')
    expect(ageValue).toBe(25)

    const tagsValue = formRef.value?.getFieldFormatValue?.('tags')
    expect(tagsValue).toEqual(['tag1', 'tag2', 'tag3'])

    // 测试嵌套字段
    const nestedField1 = formRef.value?.getFieldFormatValue?.([
      'nested',
      'field1',
    ])
    expect(nestedField1).toBe('value1')

    const nestedField2 = formRef.value?.getFieldFormatValue?.([
      'nested',
      'field2',
    ])
    ...
    // 测试数组字段
    const arrayItem0 = formRef.value?.getFieldFormatValue?.(['array', 0])
    expect(arrayItem0).toBe('item1')

    const arrayItem1 = formRef.value?.getFieldFormatValue?.(['array', 1])
    expect(arrayItem1).toBe('item2')

    // 测试 getFieldFormatValueObject
    const nameObject = formRef.value?.getFieldFormatValueObject?.('name')
    expect(nameObject).toEqual({ name: 'test' })

    const tagsObject = formRef.value?.getFieldFormatValueObject?.('tags')
    expect(tagsObject).toEqual({ tags: ['tag1', 'tag2', 'tag3'] })

    const nestedObject = formRef.value?.getFieldFormatValueObject?.([
      'nested',
      'field1',
    ])
    ...
    const allObject = formRef.value?.getFieldFormatValueObject?.()
  ...
  it('📦 getFieldsFormatValue should handle omitNil correctly', async () => {
    ...
    // 测试 omitNil=true 的情况
    const valuesWithOmitNil = formRef.value?.getFieldsFormatValue?.()
    ...
    const valuesWithoutOmitNil = formRef.value?.getFieldsFormatValue?.(
      true,
      false,
    )
  ...
  it('📦 getFieldsFormatValue should handle date formatting', async () => {
    ...
    const values = formRef.value?.getFieldsFormatValue?.()
  ...
  it('📦 getFieldsFormatValue should handle complex transforms', async () => {
    ...
    // 测试单个字段的 transform
    const priceValue = formRef.value?.getFieldFormatValue?.('price')
    expect(priceValue).toBe(100.5)

    const tagsValue = formRef.value?.getFieldFormatValue?.('tags')
    expect(tagsValue).toEqual(['tag1', 'tag2', 'tag3'])

    const statusValue = formRef.value?.getFieldFormatValue?.('status')
  ...
  it('📦 validateFieldsReturnFormatValue should work correctly', async () => {
    ...
    const validatedValues
      = await formRef.value?.validateFieldsReturnFormatValue?.()
    ...
    // 测试验证特定字段
    const validatedName
      = await formRef.value?.validateFieldsReturnFormatValue?.(['name'])
