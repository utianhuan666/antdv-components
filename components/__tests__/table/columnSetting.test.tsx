...
function fireDragEvent(ele: HTMLElement | undefined, eventName: string, data: object) {
  // @ts-expect-error dynamic event factory name mirrors Testing Library API.
  const event = createEvent[eventName](ele)
  Object.keys(data).forEach((key) => {
    // @ts-expect-error test assigns drag payload fields dynamically.
    event[key] = data[key]
  })
  fireEvent(ele, event)
}
    ...
    const html = render(
      <ProTable
        ...
        options={{
          setting: {
            children: (
              <button class="custom-setting-button">Click Me!</button>
            ),
          },
        }}
        columns={[
