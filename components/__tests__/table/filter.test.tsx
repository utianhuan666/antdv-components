...
function getFilterDropdownText(text: string) {
  const element = screen
    .queryAllByText(text)
    .find(item => item.closest('.ant-table-filter-dropdown'))
  if (!element)
    throw new Error(`Unable to find filter option: ${text}`)
  return element
}
...
describe('basicTable filter', () => {
  it('🎏 should trigger onChange when filtering with string dataIndex', async () => {
    ...
    await userEvent.click(
      document.body.querySelector('span.ant-table-filter-trigger')!,
    )
  ...
  it('🎏 should filter data locally', async () => {
    ...
    await userEvent.click(getFilterDropdownText('已上线'))
    await userEvent.click(
  ...
  it('🎏 should not trigger request when filtering locally with custom onFilter', async () => {
    ...
    await userEvent.click(getFilterDropdownText('启用'))
    await userEvent.click(
      document.body.querySelector(
        '.ant-table-filter-dropdown-btns .ant-btn-primary',
      )!,
  ...
  it('🎏 should pass filter parameters to request function with nested dataIndex', async () => {
    ...
    await waitFor(
      () => {
        expect(document.body.querySelector('.ant-table-row')).toBeInTheDocument()
      },
      { timeout: 10000 },
    )
  ...
  it('🎏 should support controlled filteredValue in columns', async () => {
    const TestComponent = defineComponent({
      name: 'ControlledFilteredValueDemo',
      setup() {
        const filteredValue = ref<string[]>(['0'])
        return () => (
          <div>
            <button
              data-testid="filter-closed"
              onClick={() => { filteredValue.value = ['0'] }}
            >
              只显示关闭
            </button>
            <button
              data-testid="filter-running"
              onClick={() => { filteredValue.value = ['1'] }}
            >
              只显示运行中
            </button>
            <button
              data-testid="filter-all"
              onClick={() => { filteredValue.value = ['0', '1'] }}
            >
              显示全部
            </button>
            <ProTable
              columns={[
                ...
                },
                {
                  ...
                  filteredValue: filteredValue.value, // 用户直接设置 filteredValue
    ...
    // 验证筛选器状态是否正确同步
    const filterTrigger = document.body.querySelector('.ant-table-filter-trigger')
  ...
  describe('reset action when use request filter', () => {
    it('🎏 should reset to defaultFilteredValue with flat columns', async () => {
      ...
      await waitFor(
        () => {
          expect(
            document.body.querySelector('.ant-table-tbody tr'),
          ).toBeInTheDocument()
          expect(screen.queryByText('项目 A')).toBeInTheDocument()
        },
        { timeout: 15000 },
