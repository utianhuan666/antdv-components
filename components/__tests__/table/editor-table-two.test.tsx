...
import { Button, Input, InputNumber, Popover } from 'antdv-next'
...
const EditorProTableDemo = defineComponent({
  name: 'EditorProTableDemo',
  props: [
    'type',
    'hideRules',
    'defaultKeys',
  ...
  setup(props: {
    ...
    const innerEditableKeys = ref<Key[]>(props.defaultKeys || [])
    const innerTableDataSource = ref<readonly DataSourceType[]>(defaultData)
    const getEditableKeys = () => props.editorRowKeys ?? innerEditableKeys.value
    const getTableDataSource = () => props.dataSource ?? innerTableDataSource.value
    const setEditorRowKeys = (updater: Key[] | ((prev: Key[]) => Key[])) => {
      const previous = getEditableKeys()
      const next = typeof updater === 'function'
        ? (updater as (p: Key[]) => Key[])(previous)
        : updater
    ...
    const setDataSource = (
      ...
      const previous = getTableDataSource()
      const next = typeof updater === 'function'
        ? (updater as (p: readonly DataSourceType[]) => readonly DataSourceType[])(previous)
        : updater
      innerTableDataSource.value = next
    ...
    return () => (
      <EditableProTable
        ...
        actionRef={actionRef}
        value={getTableDataSource()}
        onChange={setDataSource}
        editable={{
          ...props,
          type: props.type,
          editableKeys: getEditableKeys(),
          onSave: props.onSave,
...
describe('editorProTable 2', () => {
  ...
  it('🐛 title function should not show duplicate popover layers', async () => {
    const TitleWithPopover = defineComponent<{
      schema: ProColumns<DataSourceType>
      type?: string
      dom: VNodeChild
    }>({
      name: 'EditableTitleWithPopover',
      props: ['schema', 'type', 'dom'],
      setup(props) {
        ...
        return () => (
          ...
          >
            <Button type="link" onClick={() => { open.value = true }}>
              {typeof props.schema.title === 'function'
                ? '标题'
                : (props.schema.title ?? '标题')}
            </Button>
          </Popover>
        )
      },
