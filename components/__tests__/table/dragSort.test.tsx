...
async function dragAndDrop(cell: Element | undefined) {
...
*/
function mockGetBoundingClientRect(element: any, index: number) {
...
describe('dragSort', () => {
  ...
  it('🔥 [dragSort] render drag sort default handle by dragSortKey', async () => {
    ...
    interface DataSourceItemStruct {
      id: string
      name: string
    }
        ...
        columns={[
          {
            ...
            render: (dom, rowData, index) => {
              ...
              return (
                <span class="customRender">{`自定义排序[${rowData.name}-${index}]`}</span>
              )
            },
          },
          {
