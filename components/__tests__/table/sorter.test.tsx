import type { TableSortOrder as SortOrder } from 'antdv-next'
...
import { cleanup, createRef, render, screen, userEvent, waitFor } from '../testUtils'
...
describe('basicTable sorter', () => {
  ...
  it('🎏 should sort data locally', async () => {
    ...
    await waitFor(
      () => {
        ...
        expect(rows[0]!.textContent).toContain('项目 B')
        expect(rows[1]!.textContent).toContain('项目 C')
        expect(rows[2]!.textContent).toContain('项目 A')
    ...
    expect(rowsAfterMoneySort[0]!.textContent).toContain('项目 A')
    expect(rowsAfterMoneySort[1]!.textContent).toContain('项目 B')
    expect(rowsAfterMoneySort[2]!.textContent).toContain('项目 C')
    ...
    expect(rowsAfterCountSort[0]!.textContent).toContain('项目 B')
    expect(rowsAfterCountSort[1]!.textContent).toContain('项目 A')
    expect(rowsAfterCountSort[2]!.textContent).toContain('项目 C')
  ...
  it('🎏 should not trigger request when sorting locally with compare object sorter', async () => {
    ...
    await waitFor(() => {
      const rows = container.querySelectorAll('.ant-table-row')
      expect(rows[0]!.textContent).toContain('项目 A')
      expect(rows[1]!.textContent).toContain('项目 C')
      expect(rows[2]!.textContent).toContain('项目 B')
    })
  ...
  it('🎏 should sort data request', async () => {
    ...
    await waitFor(
      () => {
        const rows = container.querySelectorAll('.ant-table-row')
        expect(rows[0]!.textContent).toContain('项目 A')
        expect(rows[1]!.textContent).toContain('项目 B')
        expect(rows[2]!.textContent).toContain('项目 C')
  ...
  it('🎏 should support controlled sortOrder in columns', async () => {
    const TestComponent = defineComponent({
      name: 'ControlledSortOrderDemo',
      setup() {
        const sortOrder = ref<SortOrder | null>('descend')
        return () => (
          <ProTable
            columns={[
              {
            ...
            toolBarRender={() => [
              <button
                ...
                onClick={() => { sortOrder.value = 'descend' }}
              >
                倒序
              </button>,
              <button
                ...
                onClick={() => { sortOrder.value = 'ascend' }}
              >
                正序
              </button>,
              <button
  ...
  describe('reset action when use request sort', () => {
    ...
    it('🎏 should reset to defaultSortOrder with nested columns', async () => {
      ...
      await waitFor(
        () => {
          const rows = container.querySelectorAll('.ant-table-row')
          expect(rows[0]!.textContent).toContain('項目 B') // 95分
          expect(rows[1]!.textContent).toContain('項目 A') // 85分
          expect(rows[2]!.textContent).toContain('項目 C') // 75分
        },
        { timeout: 1000 },
      )
      ...
      await waitFor(
        () => {
          const rows = container.querySelectorAll('.ant-table-row')
          expect(rows[0]!.textContent).toContain('項目 C') // 75分
          expect(rows[1]!.textContent).toContain('項目 A') // 85分
          expect(rows[2]!.textContent).toContain('項目 B') // 95分
        },
        { timeout: 1000 },
      )
  ...
  describe('multiple column sorting', () => {
    ...
    it('🎏 should support multiple locale sort in columns', async () => {
      ...
      await waitFor(() => {
        const rows = container.querySelectorAll('.ant-table-row')
        expect(rows[0]!.textContent).toContain('项目 B')
        expect(rows[1]!.textContent).toContain('项目 A')
        expect(rows[2]!.textContent).toContain('项目 C')
      })
