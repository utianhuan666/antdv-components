...
import type { ProFormInstance } from '../../index'
...
import { act, cleanup, createRef, fireEvent, render, waitFor, waitTime } from '../testUtils'
...
describe('basicTable Search', () => {
  ...
  it('🎏 submit test', async () => {
    ...
    const dom = await (await html.findAllByText('查 询'))[0]
  ...
  it('🎏 reset test', async () => {
    ...
    const dom = await (await html.findAllByText('重 置'))[0]
        ...
        columns={[
          {
            ...
            dataIndex: 'money',
            valueType: 'money',
            formItemRender: () => <AnyInput id="formItemRender" />,
          },
          {
            title: 'Name',
            key: 'name',
  ...
  it('🎏 request load null', async () => {
    ...
    await waitFor(() => {
      expect(() => {
        // @ts-expect-error dive helper intentionally throws for null request data.
        html.dive().html()
      }).toThrowError()
    })
    html.unmount()
  ...
  it('🎏 request load more time', async () => {
    const TableDemo: FunctionalComponent<{ v: boolean }> = ({ v }) => {
      return v
        ? (
            <ProTable
              size="small"
