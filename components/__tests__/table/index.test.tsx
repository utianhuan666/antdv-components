...
import { Button, Input, Popover } from 'antdv-next'
import { afterEach, describe, expect, it, vi } from 'vitest'
import { defineComponent, ref } from 'vue'
import { ProTable, TableDropdown } from '../../index'
import {
  act,
  cleanup,
  createRef,
...
describe('basicTable', () => {
  ...
  it('🎏 fullscreen icon test when fullscreenEnabled', async () => {
    const fn = vi.fn()
    // @ts-expect-error fullscreenEnabled is readonly in DOM typings.
  ...
  it('🐛 title function should not show duplicate popover layers in ProTable', async () => {
    ...
    const columnsWithTitleFunction = [
      {
        title: (schema: any, type?: string, dom?: VNodeChild) => (
          <TitleWithPopover
            schema={schema}
            type={type ?? 'text'}
            dom={dom ?? null}
