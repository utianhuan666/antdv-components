...
import {
  ...
  createRef,
  fireEvent,
  waitForWaitTime as rawWaitForWaitTime,
  render,
  waitFor,
} from '../testUtils'
...
async function waitForWaitTime(time = 100) {
  const promise = rawWaitForWaitTime(time)
  if (vi.isFakeTimers()) {
    await act(async () => {
...
describe('editorProTable', () => {
  ...
  it('📝 EditableProTable add support nested children column', async () => {
    ...
    await act(async () => {
      (await wrapper.queryAllByText('添加一行数据'))[0]?.click()
    })
  ...
  it('📝 EditableProTable add support parentKey when newRecordType = cache', async () => {
    ...
    expect(
      wrapper.container
        ...
        ?.querySelectorAll('tr.ant-table-row')
        .length,
    ).toBe(6)
  ...
  it('📝 EditableProTable editableFormRef need rowIndex', async () => {
    ...
    try {
      // @ts-expect-error rowIndex is intentionally omitted.
      editorRef.current?.getRowData?.()
    }
    catch (error) {
      expect((error as Error).message).toBe('rowIndex is required')
    }
  ...
  it('📝 EditableProTable onValuesChange will not trigger when init', async () => {
    ...
    await act(async () => {
      (await wrapper.queryAllByText('编辑'))[0]?.click()
    })
    await waitForWaitTime(200)
  ...
  it('📝 EditableProTable support formItemProps as function', async () => {
    const formItemPropsFn = vi.fn((_row, _config) => ({
      rules: [
        {
          required: true,
          message: '此项为必填项',
  ...
  it('📝 EditableProTable support fieldProps as function', async () => {
    const fieldPropsFn = vi.fn((_row, _config) => ({
      placeholder: '请输入标题',
    }))
  ...
  it('📝 EditableProTable support columns dependencies in editable mode', async () => {
    const requestFn = vi.fn(async (_values) => {
      return [
