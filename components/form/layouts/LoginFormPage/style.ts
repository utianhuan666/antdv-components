...
export interface LoginFormPageToken extends ProAliasToken {
  componentCls: string
}

const genLoginFormPageStyle: GenerateStyle<LoginFormPageToken> = (token) => {
  return {
    [token.componentCls]: {
      ...
      '&-container': {
        ...
        backgroundImage:
          'radial-gradient(circle at 93% 100%, rgba(22,119,255,0.17) 0%, rgba(255,255,255,0.05) 23%, rgba(255,255,255,0.03) 87%, rgba(22,119,255,0.12) 109%)',
        padding: 32,
...
export function useStyle(prefixCls: string) {
  return useAntdStyle('LoginFormPage', (token) => {
    const loginFormToken: LoginFormPageToken = {
      ...token,
      componentCls: `.${prefixCls}`,
    }

    return [genLoginFormPageStyle(loginFormToken)]
  })
}
