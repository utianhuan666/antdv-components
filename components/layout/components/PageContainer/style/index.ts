...
const [sm, md, lg, xl] = [576, 768, 992, 1200].map(bp => `@media (max-width: ${bp}px)`) as [string, string, string, string]
...
const genPageContainerStyle: GenerateStyle<PageContainerToken> = (token) => {
  return {
    [token.componentCls]: {
      ...
      '& &-warp-page-header': (() => {
        const blockPad = token.layout?.pageContainer?.paddingBlockPageContainerContent ?? 24
        return {
...
export interface PageContainerComponentToken {
  paddingInlinePageContainerContent?: number
  paddingBlockPageContainerContent?: number
}

export function useStyle(prefixCls: string, componentsToken: PageContainerComponentToken | undefined) {
  return useAntdStyle('ProLayoutPageContainer', (token) => {
    const pageContainerToken: PageContainerToken = {
      ...token,
    ...
    return [genPageContainerStyle(pageContainerToken)]
  })
}
