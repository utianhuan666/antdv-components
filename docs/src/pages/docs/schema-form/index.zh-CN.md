...
description: SchemaForm 是根据 JSON Schema 来生成表单的工具。
---
...
| `shouldUpdate` | `boolean \| ((newValues: Record<string, any>, oldValues?: Record<string, any>) => boolean)` | 细粒化控制是否渲染。<br /> 为 `true` 时会自动重新渲染表单项。<br /> 为 `false` 时不会更新表单项但可以使用 [dependencies 触发更新](#结合-shouldupdatefalse-和-dependencies-触发更新)。<br /> 为 `function` 时根据返回值判断是否重新渲染表单项 |
| `title`        | `VNodeChild`                                                                                | 表单标题                                                                                                                                                                                                                                     |
| `action`       | `{ value?: ProCoreActionType }`                                                             | 用于操作表单的 action，支持 reload 等操作                                                                                                                                                                                                    |
| `formRef`      | `{ value?: ProFormInstance }`                                                               | 获取 form 实例，支持 antd form 的所有方法                                                                                                                                                                                                    |
...
| 字段名称              | 类型                                                                           | 说明                                                                                                                                                                                     |
| --------------------- | ------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `key`                 | `string \| number`                                                             | 确定这个列的唯一值，一般用于 dataIndex 重复的情况                                                                                                                                        |
| `dataIndex`           | `string \| number \| (string \| number)[]`                                     | 与实体映射的 key，数组会被转化 `[a,b] => Entity.a.b`                                                                                                                                     |
| `valueType`           | `ProFieldValueType`                                                            | 数据的渲渲染方式，我们自带了一部分，你也可以自定义 valueType                                                                                                                             |
| `title`               | `VNodeChild` \|`(props,type,dom)=> VNodeChild`                                 | 标题的内容，在 form 中是 label                                                                                                                                                           |
...
| `render`              | `(dom,entity,index, action, schema) => VNodeChild`                             | 自定义只读模式的 dom,`render` 方法只管理的只读模式，编辑模式需要使用 `formItemRender`                                                                                                    |
| `formItemRender`      | `(schema,config,form) => VNodeChild`                                           | 自定义编辑模式，返回一个 VNodeChild，会自动包裹 value 和 onChange。~~如返回 false,null,undefined 将不展示表单项~~ 请使用 dependency 组件控制是否渲染列                                   |
...
| `rowProps`            | `RowProps`                                                                     | 开启栅格化模式时传递给 Row                                                                                                                                                               |
...
<demo src="./demo/dependency.vue" title="使用 ProFormDependency"></demo>

### 高性能模式

### 使用 dependencies 触发 fieldProps/formItemProps/formItemRender 更新
...
<demo src="./demo/dependencies.vue" title="结合 shouldUpdate=false 和 dependencies 触发更新"></demo>

<demo src="./demo/dynamic-rerender.vue" title="动态控制是否重渲染"></demo>

### 列表配置 (FormList)

支持 `valueType: 'formList'` 来生成动态增减的列表表单。

<demo src="./demo/form-list-required.vue" title="FormList Required 校验"></demo>
